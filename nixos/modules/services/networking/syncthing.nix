...
let
  ...
  folders = mapAttrsToList ( _: folder: {
    ...
    pullerPauseS = folder.pullerPauseS;
    ...
    maxConcurrentWrites = folder.maxConcurrentWrites;
    disableFsync = folder.disableFsync;
    blockPullOrder = folder.blockPullOrder;
  ...
  options = mapAttrsToList (name: options: {
    listenAddress = device.globalAnnounceServer;
    globalAnnounceServer = device.globalAnnounceServer;
    globalAnnounceEnabled = device.globalAnnounceEnabled;
    localAnnounceEnabled = device.localAnnounceEnabled;
    localAnnouncePort = device.localAnnouncePort;
    localAnnounceMCAddr = device.localAnnounceMCAddr;
    maxSendKbps = device.maxSendKbps;
    maxRecvKbps = device.maxRecvKbps;
    reconnectionIntervalS = device.reconnectionIntervalS;
    relaysEnabled = device.relaysEnabled;
    relayReconnectIntervalM = device.relayReconnectIntervalM;
    startBrowser = device.startBrowser;
    natEnabled = device.natEnabled;
    natLeaseMinutes = device.natLeaseMinutes;
    natRenewalMinutes = device.natRenewalMinutes;
    natTimeoutSeconds = device.natTimeoutSeconds;
    urAccepted = device.urAccepted;
    urSeen = device.urSeen;
    urUniqueID = device.urUniqueID;
    urURL = device.urURL;
    urPostInsecurely = device.urPostInsecurely;
    urInitialDelayS = device.urInitialDelayS;
    restartOnWakeup = device.restartOnWakeup;
    autoUpgradeIntervalH = device.autoUpgradeIntervalH;
    upgradeToPreReleases = device.upgradeToPreReleases;
    keepTemporariesH = device.keepTemporariesH;
    cacheIgnoredFiles = device.cacheIgnoredFiles;
    progressUpdateIntervalS = device.progressUpdateIntervalS;
    limitBandwidthInLan = device.limitBandwidthInLan;
    releasesURL = device.releasesURL;
    overwriteRemoteDeviceNamesOnConnect = device.overwriteRemoteDeviceNamesOnConnect;
    tempIndexMinBlocks = device.tempIndexMinBlocks;
    unackedNotificationID = device.unackedNotificationID;
    trafficClass = device.trafficClass;
    setLowPriority = device.setLowPriority;
    maxFolderConcurrency = device.maxFolderConcurrency;
    crashReportingURL = device.crashReportingURL;
    crashReportingEnabled = device.crashReportingEnabled;
    stunKeepaliveStartS = device.stunKeepaliveStartS;
    stunKeepaliveMinS = device.stunKeepaliveMinS;
    stunServer = device.stunServer;
    databaseTuning = device.databaseTuning;
    maxConcurrentIncomingRequestKiB = device.maxConcurrentIncomingRequestKiB;
    announceLANAddresses = device.announceLANAddresses;
    sendFullIndexOnUpgrade = device.sendFullIndexOnUpgrade;
    connectionLimitEnough = device.connectionLimitEnough;
    connectionLimitMax = device.connectionLimitMax;
    insecureAllowOldTLSVersions = device.insecureAllowOldTLSVersions;
  }) cfg.options;
...
in {
  ...
  options = {
    services.syncthing = {
      ...
      overrideOptions = mkOption {
        type = types.bool;
        default = true;
        description = mdDoc ''
          Whether to delete the changes which are not configured via the
          [options](#opt-services.syncthing.options) option.
          If set to `false`, options changed via the web
          interface will persist and will have to change manually.
        '';
      };
      ...
      options = mkOption {
        ...
        description = mdDoc ''
          ...
          Note that you can still change options manually, but those changes
          will be reverted on restart if [overrideOptions](#opt-services.syncthing.overrideOptions)
          is enabled.
        '';
        type = types.attrsOf (types.submodule ({ name, ... }: {
          options = {
